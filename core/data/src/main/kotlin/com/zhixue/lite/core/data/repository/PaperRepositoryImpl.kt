...
internal class PaperRepositoryImpl @Inject constructor(
    ...
    private val sheetInfoDao: SheetInfoDao,
    ...
    override suspend fun syncSheetInfo(reportId: String, paperId: String) {
        try {
            networkDataSource.getSheetInfo(reportId, paperId, userRepository.token)
                .asEntity(userRepository.userId, paperId)
                .run { sheetInfoDao.insertSheetInfo(this) }
        } catch (e: Exception) {
            e.printStackTrace()
        }
    }
